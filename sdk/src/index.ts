...
export class AgentRelayClient {
  ...
  public async remember(
    agentId: string, 
    memoryContext: any, 
    options?: { version?: string; importance?: number; sourceLinks?: string[] }
  ): Promise<string> {
    ...
    if (parentBlobId) {
      try {
        const parentState = await this.recall(parentBlobId);
        if (parentState) {
    ...
    if (this.config.simulateMode) {
      ...
      try {
        ...
        const agent = typeof window === 'undefined' ? new https.Agent({ rejectUnauthorized: false }) : undefined;
        let response;
        if (isBackendRelay) {
          response = await axios.post(uploadUrl, { content: base64Content }, { httpsAgent: agent });
        } else {
  ...
  public async recall(parentBlobId: string): Promise<any> {
    ...
    try {
      ...
      const decrypted = await this.decryptBytes(encryptedBytes);
  ...
  public async restore(agentId: string): Promise<any> {
    this.log(`Initiating automated failover recovery for Agent: "${agentId}"...`);
    this.log(`Querying Sui Move list for latest registered BlobID...`);
  ...
  public async forkAgent(parentId: string, childId: string): Promise<AgentStateData> {
    ...
    if (this.config.simulateMode) {
      const parent = this.simulatedAgents.get(parentId);
      if (!parent) {
      ...
      this.log(`Building Sui transaction block to link "${childId}" to Parent: "${parentId}"...`);
